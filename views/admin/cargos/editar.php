<div class="fixed top-0 left-0 right-0 z-50 hidden w-full h-full justify-center items-center p-4 overflow-x-hidden overflow-y-auto outline-0 md:inset-0 max-h-full transition group ease-linear modal" id="modal-cargo-editar">

   <div class="opacity-60 fixed inset-0 bg-black transition modal-background"></div>

   <div class="relative w-full max-w-lg max-h-full container-form-cargos transition duration-[400ms] ease-out -translate-y-12 modal-dialog">
      <!--Modal content-->
      <div class="relative rounded-lg shadow bg-gray-800">
         ...
         <!-- Modal body -->
         <form class="" id="editar" method="POST">
            <div class="w-full mx-auto alertas "></div>
            <div class="grid gap-4 sm:grid-cols-2">
               <div class="p-6 col-span-2">
                  <label for="cargo_editar" class="block mb-2 text-gray-100 false">Cargo</label>
                  <input type="text" id="cargo_editar" class="rounded-lg border-transparent flex-1 appearance-none border w-full py-2.5 px-3 bg-gray-700 transition text-gray-300 placeholder-gray-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-azul-claro focus:border-transparent" placeholder="Ingresar cargo" name="nombre">
                  <input type="hidden" name="id" id="id_cargo">
               </div>
